        ...
        {data.map((el) => {
          return (
            <GridItem w="100%" h="100%" bg="blue.500" key={el.id}>
                {el.rating?<Products name={el.name} category={el.category} option={el.option} rating={el.rating} ratingCount={el['rating-count']}  />:<Products name={el.name} category={el.category} option={el.option} />}
            </GridItem>
          );
        })}
      </Grid>
